...
|-------|---------|------|-------------|
| 0 | **Provider** | — | Pluggable storage backend (in-memory for dev, cloud for prod) |
...
Starts the Djinn (ports 9001–9005, proxy range 9100–9200) and a Go greeter service pre-registered in the Registry.
...
coordin8/
  ...
  djinn/                       Rust workspace (core services)
    crates/
      ...
      coordin8-provider-local/ In-memory provider
...
| Space | Planned | Distributed reactive tuple store (`out`, `take`, `read`, `watch`) |
| AWS Provider | Planned | DynamoDB, SQS, EventBridge backend |
