...
> **Status: COMPLETE (merged to main, 2026-09-06).** All 6 items done. Items 1-5 via #20, #21, #22, a corrective consolidation PR #23, and #30 for item 5 (#21/#22 were stacked on non-`main` branches, so merging them didn't land on `main` until #23; see `feedback_stacked_pr_base_branches` lesson). Item 6 (gRPC security via JWT) via [PR #34](https://github.com/jsarman/coordin8/pull/34) — see `.claude/plans/grpc-security/PRD.md` for the full plan and implementation writeup. This roadmap has no open items; further hardening work should start a new topic folder rather than reopening this one. See `session-1-complete.md` for the items 1-5 implementation writeup. Docker-centric and near-term — a precursor to, not a duplicate of, `.claude/plans/phase3-cloud-topology/` (which is the full AWS-serverless endgame).
...
### 6. gRPC security via JWT — ✅ DONE (via #34, 2026-09-06)

Full plan and implementation: `.claude/plans/grpc-security/PRD.md`. All of this item's original open questions were resolved there as decisions: signing (HS256 shared-secret for v1, not RS256/JWKS), token issuance (static, CLI-minted, no live issuance service — deliberately avoids the leasing-style bootstrap cycle, since a service reads its own token from config the same way it already reads `COORDIN8_REGISTRY`), which services validate (every service independently, via a shared `coordin8-auth` interceptor crate — Coordin8 has no gateway/ingress choke point to centralize this at), scope (authentication only for v1, no RBAC yet), and mTLS/TLS (explicitly deferred, separate future work, not blocking JWT auth from shipping). Shipped: Rust core, CLI `mint-token`, Go/Java/Node SDK token support, and an opt-in auth overlay for every example — all live-verified end-to-end.
