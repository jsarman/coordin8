...
> **Status: COMPLETE (merged to main, 2026-09-06), PR #27.** Examples re-validated against the rebuilt split-mode stack (PR #28: market-watch/double-entry pass unmodified, auction-house needed 3 compose fixes). Supersedes and reverses part of `.claude/plans/registry-bootstrap/` Phase 1b. Java/Node SDK work (registry-bootstrap Phases 3/4) stays paused until Node/Java get their own pass under this model — Java SDK also needs a fix for a leftover hardcoded LeaseMgr port (see Remaining follow-ups). See `session-1-complete.md` in this folder for the full writeup.
...
5. **Re-validate** — ✅ DONE, including the examples. Full `cargo test --all` + `fmt` + `clippy` clean. Both `docker-compose.split.yml` (5 containers now, no more `lease` service) and `docker-compose.yml` (bundled monolith) rebuilt and live-verified: register/renew/cancel against Registry's and Space's independently-embedded LeaseManagers directly (proved isolation — a Space-granted lease ID is genuinely unknown to Registry, `NotFound`), hello-coordin8 Go greeter service+client round-trip in bundled mode. **market-watch and double-entry pass unmodified against the split-mode stack** (double-entry needs `ADVERTISE_HOST=host.docker.internal` on macOS host — a pre-existing environment quirk, not a leasing bug). **auction-house needed 3 example-local compose fixes** (stale port 9001 healthcheck, stale `DJINN_HOST` env var, missing `COORDIN8_ADVERTISE_HOST`) — shipped in PR #28, live-verified full flow (create → bid → TTL expiry → settlement → SOLD).
...
- **Constraint, not a bug: single replica per namespace when using a shared DynamoDB lease table.** `grantor_host`/`grantor_port` are stamped with the granting *process's own* advertise address, not a service-level VIP/DNS name. If N replicas of the same service (e.g. Space) share a `coordin8_leases_space` table, a lease is only renewable at the specific replica that granted it — if that replica dies, a surviving replica can see the record in the shared table but the holder has no live grantor to reach for it, and the lease lapses even though the service as a whole is still up. Each replica's own reaper also independently scans the same shared table and broadcasts into its own in-process cascade, so N replicas racing `drain_expired` do duplicated cleanup work rather than corruption (handlers are idempotent) — wasteful, not unsafe. Found in code review (`coordin8-review-2026-09-07.md`); documented here rather than fixed, since a real fix (stamping a shared service-level address instead of the replica's own, or centralizing the reaper) is a bigger change than this PRD's scope and isn't worth building until a real multi-replica-per-namespace deployment actually needs it. See `djinn/providers/dynamo/README.md`'s Notes section for the same constraint stated where a deployer would actually look for it.
- **Java SDK has a live leftover reference to the old centralized LeaseMgr**, found while validating auction-house (whose `auction-service` participant is Java): `sdks/java/src/main/java/io/coordin8/DjinnClient.java:40` hardcodes a standalone `leasePort=9001` channel that no longer exists; `LeaseClient.java:63-69`'s `keepAlive` scheduled renewal silently swallows the resulting connection failure. Auction-service's registration lease stops renewing ~15-30s after boot and silently expires (didn't crash the demo since nothing discovers it via Registry in that flow, but it's a real, reproducible bug). Fix when registry-bootstrap Phase 3 (Java SDK) resumes — don't fix piecemeal now.
