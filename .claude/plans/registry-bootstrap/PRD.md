...
> **Status: In progress.** Phases 1, 1b, and 2 done and merged into `feat/registry-only-bootstrap` (not yet PR'd). Phases 3 (Java SDK), 4 (Node SDK), 5 (re-validate every example, including finally running auction-house on split mode) not started. Found while trying to run the auction-house demo against `docker-compose.split.yml` (`.claude/plans/hardening-roadmap/`, item 2) — auction-service/settlement-engine/auction-board all take a single `DJINN_HOST` and dial fixed ports on it, which only works for monolith topology. Investigated and rejected two smaller fixes (a Proxy-forwarding sidecar; per-service host overrides in each SDK) before landing on this as the actually-correct one — see Non-Goals for why.
